...
package_install() {
  ...
  apt-get -y update -q 1> /dev/null

  # Install required packages
  echo " * Installing apt-get packages..."
  if ! apt-get -y -q --allow-downgrades --allow-remove-essential --allow-change-held-packages -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confnew install --fix-missing --fix-broken ${apt_package_install_list[@]}; then
    echo " * Installing apt-get packages returned a failure code, cleaning up apt caches then exiting"
    apt-get clean -q 1> /dev/null
    return 1
  fi
...
configure() {
  ...
  cp -f "${DIR}/php${PHPVERSION}-upstream.conf" "/etc/nginx/upstreams/php56.conf"
  echo " * Copied ${DIR}/php${PHPVERSION}-upstream.conf to /etc/nginx/upstreams/php56.conf"
  ...
  if [[ -e /srv/config/php-config/mailhog.ini ]]; then
    cp "/srv/config/php-config/mailhog.ini" "/etc/php/${PHPVERSION}/mods-available/mailhog.ini"
