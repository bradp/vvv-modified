...
function fetch_tideways_repo() {
    if [[ ! -d /var/local/tideways-php/.git ]]; then
        ...
        git clone "https://github.com/tideways/php-xhprof-extension" /var/local/tideways-php -q
    else
...
function install_php_sqlite() {
    ...
    apt-get -y -qq --allow-downgrades --allow-remove-essential --allow-change-held-packages -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confnew install --fix-missing --fix-broken "${packages[@]}"
}
...
function install_xhgui_frontend() {
    ...
    if [[ ! -d "/srv/www/default/php-profiler" ]]; then
        ...
        noroot composer require -q --no-update perftools/php-profiler
        noroot composer require -q --no-update perftools/xhgui-collector
