...
install_mongodb() {
    ...
    apt-get -y update -qq 1> /dev/null
    echo " * Installing apt-get packages"
    apt_package_install_list=(
        mongodb-org
        re2c
    )
    if ! apt-get -y -qq --allow-downgrades --allow-remove-essential --allow-change-held-packages -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confnew install --fix-missing --fix-broken ${apt_package_install_list[@]}; then
        echo " * Installing apt-get packages returned a failure code, cleaning up apt caches then exiting"
        apt-get clean -qq 1> /dev/null
        return 1
    fi
}
