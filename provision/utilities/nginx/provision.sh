...
function nginx_register_packages() {
  ...
  local APTSOURCE="/srv/provision/utilities/nginx/sources-${OSID,,}-${OSCODENAME,,}.list"
  if [ -f "${APTSOURCE}" ]; then
  ...
  if ! vvv_apt_keys_has 'nginx'; then
    ...
    apt-key add /srv/provision/utilities/nginx/apt-keys/nginx_signing.key
  fi
...
function nginx_setup() {
  ...
  vvv_info " * Copying /srv/provision/utilities/nginx/config/nginx.conf to /etc/nginx/nginx.conf"
  cp -f "/srv/provision/utilities/nginx/config/nginx.conf" "/etc/nginx/nginx.conf"

  vvv_info " * Copying /srv/provision/utilities/nginx/config/nginx-wp-common.conf to /etc/nginx/nginx-wp-common.conf"
  cp -f "/srv/provision/utilities/nginx/config/nginx-wp-common.conf" "/etc/nginx/nginx-wp-common.conf"
  ...
  vvv_info " * Rsync'ing /srv/provision/utilities/nginx/config/sites/ to /etc/nginx/custom-sites"
  rsync -rvzh --delete "/srv/provision/utilities/nginx/config/sites/" "/etc/nginx/custom-sites/" 1> /dev/null
