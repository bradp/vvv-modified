...
function vvv_register_packages() {
  ...
  local APTSOURCE="/srv/provision/utilities/vvv/sources-${OSID,,}-${OSCODENAME,,}.list"
  if [ -f "${APTSOURCE}" ]; then
  ...
  if ! vvv_apt_keys_has 'Varying Vagrant Vagrants'; then
    ...
    apt-key add /srv/provision/utilities/vvv/apt-keys/varying-vagrant-vagrants_keyserver_ubuntu.key 1> /dev/null
  fi
...
function apt_hash_missmatch_fix() {
  if [ ! -f "/etc/apt/apt.conf.d/99hashmismatch" ]; then
    vvv_info " * Copying /srv/provision/utilities/vvv/apt-conf-d/99hashmismatch to /etc/apt/apt.conf.d/99hashmismatch"
    cp -f "/srv/provision/utilities/vvv/apt-conf-d/99hashmismatch" "/etc/apt/apt.conf.d/99hashmismatch"
  fi
}
