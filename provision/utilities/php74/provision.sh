...
configure() {
  ...
  vvv_info " * Copied ${DIR}/php${PHPVERSION}-upstream.conf to /etc/nginx/upstreams/php74.conf"

  # Copy php-fpm configuration from local
  cp -f "${DIR}/php${PHPVERSION}-fpm.conf" "/etc/php/${PHPVERSION}/fpm/php-fpm.conf"
  vvv_info " * Copied ${DIR}/php${PHPVERSION}-fpm.conf to /etc/php/${PHPVERSION}/fpm/php-fpm.conf"

  cp -f "${DIR}/php${PHPVERSION}-www.conf" "/etc/php/${PHPVERSION}/fpm/pool.d/www.conf"
  vvv_info " * Copied ${DIR}/php${PHPVERSION}-www.conf to /etc/php/${PHPVERSION}/fpm/pool.d/www.conf"

  cp -f "${DIR}/php${PHPVERSION}-custom.ini" "/etc/php/${PHPVERSION}/fpm/conf.d/php-custom.ini"
  vvv_info " * Copied ${DIR}/php${PHPVERSION}-custom.ini to /etc/php/${PHPVERSION}/fpm/conf.d/php-custom.ini"

  cp -f "/srv/config/php-config/opcache.ini" "/etc/php/${PHPVERSION}/fpm/conf.d/opcache.ini"
  vvv_info " * Copied /srv/config/php-config/opcache.ini to /etc/php/${PHPVERSION}/fpm/conf.d/opcache.ini"

  cp -f "/srv/config/php-config/xdebug.ini" "/etc/php/${PHPVERSION}/mods-available/xdebug.ini"
  vvv_info " * Copied /srv/config/php-config/xdebug.ini to /etc/php/${PHPVERSION}/mods-available/xdebug.ini"
  ...
  if [[ -e /srv/config/php-config/mailcatcher.ini ]]; then
    ...
    vvv_info " * Copied /srv/config/php-config/mailcatcher.ini to /etc/php/${PHPVERSION}/mods-available/mailcatcher.ini"
  ...
  if [[ -e /srv/config/php-config/mailhog.ini ]]; then
    ...
    vvv_info " * Copied /srv/config/php-config/mailhog.ini to /etc/php/${PHPVERSION}/mods-available/mailhog.ini"
  fi

  vvv_info " * Restarting php${PHPVERSION}-fpm service "
...
vvv_info " * Restoring the default PHP CLI version ( ${DEFAULTPHP} )"
...
vvv_info " * PHP ${PHPVERSION} provisioning complete"
