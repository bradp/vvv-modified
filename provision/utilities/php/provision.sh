...
function php_register_packages() {
  ...
  local APTSOURCE="/srv/provision/utilities/php/sources-${OSID,,}-${OSCODENAME,,}.list"
  if [ -f "${APTSOURCE}" ]; then
  ...
  cp -f "/srv/provision/utilities/php/ondrej-ppa-pin" "/etc/apt/preferences.d/ondrej-ppa-pin"

  if ! vvv_apt_keys_has 'Ondřej'; then
    # Apply the PHP signing key
    vvv_info " * Applying the Ondřej PHP signing key..."
    apt-key add /srv/provision/utilities/php/apt-keys/ondrej_keyserver_ubuntu.key 1> /dev/null
  fi
