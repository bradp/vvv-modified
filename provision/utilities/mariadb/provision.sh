...
function mariadb_register_packages() {
  ...
  vvv_info " * Copying /srv/provision/utilities/mariadb/config/vvv-core.cnf to /etc/mysql/conf.d/vvv-core.cnf"
  cp -f "/srv/provision/utilities/mariadb/config/vvv-core.cnf" "/etc/mysql/conf.d/vvv-core.cnf"

  if ! vvv_apt_keys_has 'MariaDB'; then
    # Apply the MariaDB signing keyg
    vvv_info " * Applying the MariaDB signing key..."
    apt-key add /srv/provision/utilities/mariadb/apt-keys/mariadb.key 1> /dev/null
  fi

  local OSID=$(lsb_release --id --short)
  local OSCODENAME=$(lsb_release --codename --short)
  local APTSOURCE="/srv/provision/utilities/mariadb/sources-${OSID,,}-${OSCODENAME,,}.list"
  if [ -f "${APTSOURCE}" ]; then
...
function mysql_setup() {
  ...
  cp -f "/srv/provision/utilities/mariadb/config/my.cnf" "/etc/mysql/my.cnf"
  vvv_info " * Copied /srv/provision/utilities/mariadb/config/my.cnf to /etc/mysql/my.cnf"

  cp -f  "/srv/provision/utilities/mariadb/config/root-my.cnf" "/home/vagrant/.my.cnf"
  chmod 0644 "/home/vagrant/.my.cnf"
  vvv_info " * Copied /srv/provision/utilities/mariadb/config/root-my.cnf to /home/vagrant/.my.cnf"
